...
Spec (from MBO.md, with the q-batch update):
  - 240 total designs = 40 initial LHS (already in results.csv) + 200 BO
...
Run:
    python mbo/run_mbo.py                 # auto-loop until 200 BO designs
    python mbo/run_mbo.py --q 3 --cpus 4  # batch of 3, 4 cpus per solver
    python mbo/run_mbo.py --max-iters 6   # smoke test (cap specimen count)

Resume:
    Just re-run.  The driver reads results.csv, derives next SPECIMEN_NUM
    from `max(SPECIMEN_NUM) + 1`, and continues until budget is reached.
...
TOTAL_BUDGET   = 240
INIT_BUDGET    = 40
BO_BUDGET      = TOTAL_BUDGET - INIT_BUDGET   # 200
BATCH_Q        = 3
CPUS_PER_JOB   = 4
...
RESULTS_HEADER_16 = [
    'SPECIMEN_NUM',
    'MAX_FI_ALL', 'MAX_FI_PINCHING', 'MAX_FI_FOLDING', 'MAX_SE',
